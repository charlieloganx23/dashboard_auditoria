# Criando o código completo do painel de auditoria financeira avançado
codigo_completo = '''
...
def autenticar_usuario():
    ...
    if not st.session_state.authenticated:
        st.title("🔐 Acesso ao Painel de Auditoria")
        usuario = st.text_input("Usuário:")
        senha = st.text_input("Senha:", type="password")
        
        if st.button("Entrar"):
            # Hash simples para demonstração (em produção, use autenticação robusta)
            if hashlib.md5(f"{usuario}:{senha}".encode()).hexdigest() == "5d41402abc4b2a76b9719d911017c592":  # hello:hello
        ...
        st.info("Demo: usuário 'hello', senha 'hello'")
        return False
    return True
...
def registrar_log(acao, detalhes=""):
    ...
    usuario = st.session_state.get('usuario', 'Anônimo')
    log_entry = f"{timestamp} | {usuario} | {acao} | {detalhes}\\n"
    
    try:
        with open("audit_log.txt", "a", encoding="utf-8") as f:
            f.write(log_entry)
    except:
        pass  # Em caso de erro, continua sem logging
...
registrar_log("Acesso ao painel", "Dados carregados com sucesso")
...
if uploaded_file:
    ...
    registrar_log("Upload de arquivo", uploaded_file.name)
...
# Filtro de valor mínimo para análise
...
with tab2:
    ...
    if not df_fin_filtrado.empty:
        ...
        # Identificar registros com outliers
        ...
        if not outliers.empty:
            ...
            registrar_log("Análise de outliers", f"{len(outliers)} outliers detectados")
        else:
            st.success("✅ Nenhum outlier detectado com o threshold atual.")

# TAB 3: Painel de Riscos
with tab3:
    ...
    if riscos:
        ...
        registrar_log("Análise de riscos", f"{len(riscos)} riscos identificados")
    else:
...
with tab5:
    ...
    with col1:
        ...
        if not df_fin_filtrado.empty:
            ...
            # Índice de eficiência (relação entre físico e financeiro)
    ...
    # Ranking de órgãos por eficiência
    if not df_fin_filtrado.empty:
...
with tab6:
    ...
    with col1:
        if not df_fin_filtrado.empty:
            ...
            registrar_log("Download CSV", "Dados financeiros")
    ...
    with col2:
        ...
        if not df_fin_filtrado.empty:
            ...
            registrar_log("Download Excel", "Relatório completo")
    
    with col3:
        # Log de atividades
        if st.button("📜 Ver Log de Atividades"):
            try:
                with open("audit_log.txt", "r", encoding="utf-8") as f:
                    log_content = f.read()
                st.text_area("Log de Atividades", log_content, height=200)
            except:
...
with open('painel_auditoria_avancado.py', 'w', encoding='utf-8') as f:
    f.write(codigo_completo)

print("Código do painel avançado de auditoria financeira criado com sucesso!")
print("Arquivo salvo como: painel_auditoria_avancado.py")