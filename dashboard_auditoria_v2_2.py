...
def formatar_moeda_milhoes(valor):
    """Formata valores em milhões para KPIs"""
    if pd.isnull(valor) or valor == 0:
        return "R$ 0,00M"
...
<div style="background: linear-gradient(135deg, #6c757d 0%, #495057 100%); color: white; padding: 1rem; border-radius: 8px; margin-bottom: 2rem; text-align: center;">
    ...
    <strong>Filtro de Valor:</strong> Acima de {formatar_moeda(valor_minimo)}
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    # Calcular KPIs dinâmicos baseados no filtro de órgão
    if orgaos_selecionados:
        # Filtrar programas pelos órgãos selecionados
        programas_filtrados = df_prog[df_prog['Nome_orgao'].isin(orgaos_selecionados)]
        total_programas = programas_filtrados['programa_codigo'].nunique()
        
        # Filtrar ações pelos códigos de UO dos órgãos selecionados
        codigos_uo_filtrados = programas_filtrados['uo_codigo'].unique()
        acoes_filtradas = df_acao[df_acao['uo_codigo'].isin(codigos_uo_filtrados)]
        total_acoes = acoes_filtradas['acao_codigo'].nunique()
    else:
        # Se nenhum órgão selecionado, mostrar totais gerais
        total_programas = df_prog['programa_codigo'].nunique()
        total_acoes = df_acao['acao_codigo'].nunique()
    ...
    with col1:
        ...
        <div class="metric-card">
            ...
            <h2 style="color: #1e3c72; margin: 0.5rem 0 0 0;">{formatar_numero_br(total_programas)}</h2>
            <p style="margin: 0.25rem 0 0 0; color: #6c757d; font-size: 0.9rem;">
                {'Filtrado por órgão' if orgaos_selecionados else 'Total geral'}
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="metric-card">
            ...
            <h2 style="color: #1e3c72; margin: 0.5rem 0 0 0;">{formatar_numero_br(total_acoes)}</h2>
            <p style="margin: 0.25rem 0 0 0; color: #6c757d; font-size: 0.9rem;">
                {'Filtrado por órgão' if orgaos_selecionados else 'Total geral'}
            </p>
        </div>
    ...
    with col3:
        ...
        <div class="metric-card">
            ...
            <h2 style="color: #1e3c72; margin: 0.5rem 0 0 0;">{formatar_moeda_milhoes(valor_empenhado)}</h2>
            <p style="margin: 0.25rem 0 0 0; color: #6c757d; font-size: 0.9rem;">
                Período filtrado
            </p>
        </div>
    ...
    with col4:
        ...
        <div class="metric-card">
            ...
            <h2 style="color: {cor_exec}; margin: 0.5rem 0 0 0;">{perc_exec:.1f}%</h2>
            <p style="margin: 0.25rem 0 0 0; color: #6c757d; font-size: 0.9rem;">
                Liquidado/Empenhado
            </p>
        </div>
    ...
    with col5:
        ...
        <div class="metric-card">
            ...
            <h2 style="color: {cor_rest}; margin: 0.5rem 0 0 0;">{formatar_numero_br(total_restricoes)}</h2>
            <p style="margin: 0.25rem 0 0 0; color: #6c757d; font-size: 0.9rem;">
                Período filtrado
            </p>
        </div>
    ...
    if not df_fin_filtrado.empty:
        ...
        # Preparar dados para o gráfico com formatação
        df_ano_melted = df_ano.melt(
            id_vars=['ano', 'quadrimestre'], 
            value_vars=['valor_empenhado', 'valor_liquidado_ano', 'valor_pago'],
            var_name='tipo_valor', 
            value_name='valor'
        )
        ...
        tipo_map = {
            ...
            'valor_liquidado_ano': 'Liquidado',
            'valor_pago': 'Pago'
        }
        df_ano_melted['tipo_valor'] = df_ano_melted['tipo_valor'].map(tipo_map)
        ...
        fig = px.bar(
            ...
            color='tipo_valor',
            facet_col='ano',
            labels={'valor': 'Valor (R$)', 'quadrimestre': 'Quadrimestre', 'tipo_valor': 'Tipo'},
            title="📊 Execução Financeira Comparativa por Período",
            color_discrete_sequence=['#1e3c72', '#2a5298', '#3d5aa1'],
            text='valor'
        )
        
        # Formatar os rótulos dos valores
        fig.update_traces(
        ...
        # Formatar hover para mostrar valores em reais
        fig.update_traces(
        ...
        # Gráfico de Pizza com valores formatados
        st.markdown('<div class="chart-container">', unsafe_allow_html=True)
        col1, col2 = st.columns(2)
        
        with col1:
            df_orgao = df_fin_filtrado.groupby('Nome_orgao')['valor_pago'].sum().reset_index()
            ...
            fig_pie = px.pie(
                ...
                values='valor_pago', 
                names='Nome_orgao',
                title="💰 Distribuição de Pagamentos por Órgão (Top 10)",
                color_discrete_sequence=px.colors.qualitative.Set3
            )
            
            # Formatar os rótulos do gráfico de pizza
            fig_pie.update_traces(
                textposition='inside',
                textinfo='percent+label',
                hovertemplate='<b>%{label}</b><br>' +
                             'Valor: ' + df_orgao['valor_pago'].apply(formatar_moeda).tolist()[0] + '<br>' +
        ...
        with col2:
            ...
            df_top_programas = df_fin_filtrado.groupby('programa_codigo')['valor_empenhado'].sum().reset_index()
            df_top_programas = df_top_programas.sort_values('valor_empenhado', ascending=True).tail(10)
            ...
            fig_bar_h = px.bar(
                ...
                y='programa_codigo',
                orientation='h',
                title="📈 Top 10 Programas por Valor Empenhado",
                color='valor_empenhado',
                color_continuous_scale='Blues',
                text='valor_empenhado'
            )
...
with tab2:
    ...
    if not df_fin_filtrado.empty:
        ...
        if not outliers.empty:
            ...
            # Adicionar rótulos de valores nos pontos anômalos
            anomalias_df = df_outliers[df_outliers[outlier_cols].any(axis=1)]
            if not anomalias_df.empty:
                fig_scatter.add_trace(
                    go.Scatter(
                        x=anomalias_df['valor_empenhado'],
                        y=anomalias_df['valor_liquidado_ano'],
                        mode='markers+text',
                        text=[formatar_moeda_milhoes(v) for v in anomalias_df['valor_empenhado']],
            ...
            colunas_monetarias = ['valor_empenhado', 'valor_liquidado_ano', 'valor_pago']
            for col in colunas_monetarias:
                if col in outliers_display.columns:
                    outliers_display[col] = outliers_display[col].apply(formatar_moeda)
            
            colunas_exibir = ['programa_codigo', 'acao_codigo', 'Nome_orgao'] + colunas_monetarias
            st.dataframe(outliers_display[colunas_exibir], use_container_width=True)
        else:
            st.markdown("""
        ...
        for i, risco in enumerate(riscos):
            ...
            with st.expander(f"📊 Análise Detalhada do Risco {i+1}"):
                if isinstance(risco['dados'], pd.DataFrame):
                    ...
                    dados_display = risco['dados'].copy()
                    colunas_monetarias = ['valor_empenhado', 'valor_liquidado_ano', 'valor_pago', 'valor_restos_a_pagar']
                    for col in colunas_monetarias:
                        if col in dados_display.columns:
                            dados_display[col] = dados_display[col].apply(formatar_moeda)
...
with tab4:
    ...
    if not df_fin_filtrado.empty:
        ...
        # Adicionar linhas com rótulos de valores
        cores = ['#1e3c72', '#28a745', '#dc3545']
        nomes = ['Empenhado', 'Liquidado', 'Pago']
        colunas = ['valor_empenhado', 'valor_liquidado_ano', 'valor_pago']
        ...
        for col, nome, cor in zip(colunas, nomes, cores):
            fig_timeline.add_trace(go.Scatter(
                ...
                marker=dict(size=8),
                text=[formatar_moeda_milhoes(v) for v in df_timeline[col]],
                textposition="top center",
        ...
        if len(df_timeline) > 1:
            ...
            # Formatar rótulos
            fig_sazon.update_traces(
                texttemplate=[formatar_moeda_milhoes(v) for v in df_sazon['valor_pago']],
                textposition='outside',
                textfont_size=12
            )
...
with tab5:
    ...
    with col1:
        ...
        if not df_fin_filtrado.empty:
            ...
            <div class="metric-card">
                ...
                <p style="margin: 0.5rem 0 0 0; color: #6c757d;">Liquidado: {formatar_moeda_milhoes(df_fin_filtrado['valor_liquidado_ano'].sum())}</p>
            </div>
            """, unsafe_allow_html=True)
            ...
            if not df_fis_filtrado.empty:
                ...
                <div class="metric-card">
                    ...
                    <p style="margin: 0.5rem 0 0 0; color: #6c757d;">Ações integradas: {formatar_numero_br(acoes_com_ambos)}</p>
                </div>
                """, unsafe_allow_html=True)
    ...
    if not df_fin_filtrado.empty:
        ...
        # Adicionar rótulos de percentual
...
with tab6:
    ...
    if not df_fin_filtrado.empty:
        ...
        valor_liquidado = df_fin_filtrado['valor_liquidado_ano'].sum()
        valor_pago = df_fin_filtrado['valor_pago'].sum()
        perc_exec = (valor_liquidado / valor_total * 100) if valor_total > 0 else 0
        
        sumario.append(f"**Execução Orçamentária:** No período analisado, foram empenhados {formatar_moeda(valor_total)}.")
        sumario.append(f"**Performance:** O percentual de execução atingiu {perc_exec:.1f}%, com {formatar_moeda(valor_liquidado)} liquidados.")
        sumario.append(f"**Pagamentos:** Foram efetivados pagamentos no valor de {formatar_moeda(valor_pago)}.")
    ...
    # Sumário com fundo branco e fonte azul institucional
    st.markdown(f"""
    <div style="
        background: #fff;
        padding: 2rem;
    ...
    with col1:
        if not df_fin_filtrado.empty:
            # Preparar dados formatados para exportação
            df_export = df_fin_filtrado.copy()
            colunas_monetarias = ['valor_empenhado', 'valor_liquidado_ano', 'valor_pago', 'valor_restos_a_pagar']
            for col in colunas_monetarias:
                if col in df_export.columns:
                    df_export[f'{col}_formatado'] = df_export[col].apply(formatar_moeda)
    ...
    with col2:
        if not df_fin_filtrado.empty:
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                df_fin_export = df_fin_filtrado.copy()
                df_fis_export = df_fis_filtrado.copy() if not df_fis_filtrado.empty else pd.DataFrame()
                df_rest_export = df_rest_filtrado.copy() if not df_rest_filtrado.empty else pd.DataFrame()
                
                df_fin_export.to_excel(writer, index=False, sheet_name='Financeiro')
                if not df_fis_export.empty:
                    df_fis_export.to_excel(writer, index=False, sheet_name='Físico')
                if not df_rest_export.empty:
                    df_rest_export.to_excel(writer, index=False, sheet_name='Restrições')
...
<div class="footer">
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
        ...
        <div style="text-align: right;">
            ...
            <p style="margin: 0.25rem 0 0 0; opacity: 0.6;">Versão 2.2 | Desenvolvido com Streamlit</p>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)
